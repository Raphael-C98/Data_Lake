...
def process_song_data(spark, input_data, output_data):
    ...
    Description: Read in songs data from json files.
                 Outputs songs and artists dimension tables
    ...
    songs_table.write.partitionBy("year", "artist_id")\
               .parquet("{}songs/songs_tbale.parquet".format(output_data))

    # extract columns to create artists table
    artists_table = df.select(col("artist_id"), col("name"),
                              col("location"), col("latitude"),
                              col("longitude"))\
                      .distinct()

    # write artists table to parquet files
    artists_table.write\
        .parquet("{}artist/artists_table.parquet".format(output_data))
...
def process_log_data(spark, input_data, output_data):
    ...
    artists_table = df.select(col("artist_id"), col("name"), col("location"),
                              col("latitued"), col("longitude")).distinct()
    ...
    song_df = spark.sql("SELECT DISTINCT song_id,
                        artist_id, artist_name FROM song_df_table")

    # extract columns from joined song and log datasets
    # to create songplays table
    songplays_table = df.join(song_df, song_df.name == df.artist, "inner")\
                        .distinct()\
                        .select(col("start_time"), col("user_id"),
                                col("level"),
...
def main():
    ...
    output_data = "https://us-east-1.console.aws.amazon.com/s3/object/aws-emr-resources-216524871409-us-west-2?region=us-west-2&prefix=notebooks/e-2TEKUHAQ440O5A2G4PULR085P/Sparkify.ipynb"
